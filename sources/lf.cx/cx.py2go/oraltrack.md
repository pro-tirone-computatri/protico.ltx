...
### 1 py2go:Python für Nebenbei-Selbstlerner
...
5. Sie sollen so nicht mehr als 10 min pro Tag sollen aufwenden müssen, um systematisch und praktisch kodieren zu lernen.
...
2. Kopieren Sie *py2go-00.py* in Ihr Abrbeitsverzeichnis und geben Sie dann `python3 py2go-00.py` in Ihre Shell ein. Sie sollten danach ein *Hello World* sehen.
3. Öffnen Sie Ihre *vscode* bzw. *vscodium* Instanz:
  1. Installieren Sie die Extensionen *Python* und *Python-Debugger* von *ms-python*.
  2. Öffnen Sie die Datei *py2go-00.py* i= Laden Sie sie in ein VSCode-Fenster.
  3. Klicken Sie rechts oben auf das Dreieck 'Run Python File'. (Variante: Öffnen 
     ...
     gesonderten Fenster) dieselbe Ausgabe bekommen, wie unte 2.
